...
const CompletedTask = () => {
  ...
  if (!myAllTask.length) {
    return (
      <div className="dark:bg-gray-900 p-5 py-40 md:pb-44 flex justify-center">
        <h1 className="text-3xl text-yellow-500">
          No Task Added!.
          <span className="text-blue-500 underline">
            <Link to="/addTask"> Please add any Task</Link>
          </span>
        </h1>
      </div>
    );
  ...
  return (
    <div className=" md:py-20 p-2 md:px-10 sm:p-4  dark:bg-black text-white">
      <h2 className="mb-8 text-2xl text-center font-semibold leading-tight text-black dark:text-white">
        Completed Task
      </h2>
      <div className="overflow-x-auto">
        <table className="w-full p-6 text-sm text-left">
          ...
          <tbody className="border dark:bg-gray-900 dark:border-gray-700">
            {myAllTask?.map((task, i) => (
              <tr key={task._id} className="border dark:border-gray-700">
                <td className="px-3 text-2xl font-medium dark:text-white text-black">
                  {i + 1}
                </td>
                ...
                <td className="px-3 py-2">
                  {task.completed === "true" ? (
                    <button className="px-2 py-3 rounded bg-blue-500 text-white">
                      Completed
                    </button>
                  ) : (
                    <Link to="/myTask">
                ...
                <td className="px-3 py-2">
                  {task.completed === "true" ? (
                    <button
                      onClick={() => handleDeletingMyTask(task._id)}
                      className="px-2 py-3 rounded bg-red-500 text-white"
                    >
                      Delate
                    </button>
                  ) : (
